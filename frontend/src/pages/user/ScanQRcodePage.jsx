...
import {
  QrCode, Camera, CheckCircle2, Package, Barcode,
  Search, X, Clock, User, TrendingUp, Upload, Video, VideoOff
} from 'lucide-react';
...
const QRScannerUser = () => {
  ...
  const [stats, setStats] = useState({ todayScans: 0, totalScans: 0, uniqueProducts: 0 });
  ...
  const loadStats = () => {
    ...
    setStats({ todayScans, totalScans: history.length, uniqueProducts });
  };
  ...
  const searchProduct = async (code) => {
    ...
    try {
      ...
      if (status === 404) {
        toast.error('QR Code tidak ditemukan di database!');
      } else if (status === 401) {
        toast.error('Sesi Anda telah berakhir. Silakan login kembali.');
      } else if (status === 422) {
        toast.error('Format QR Code tidak valid!');
      } else {
        toast.error(err.response?.data?.message || 'Gagal memindai QR Code. Silakan coba lagi.');
      }
      return false;
    } finally {
      setLoading(false);
  ...
  const handleManualScan = async () => {
    if (!manualCode.trim()) {
      toast.warning('Masukkan kode QR atau barcode terlebih dahulu!');
      return;
    }
        ...
        qrScannerRef.current = new QrScanner(
          ...
          async (result) => {
            if (result?.data && !loading) {
              setScanning(false);
              const ok = await searchProduct(result.data);
              if (ok) {
                stopCamera();
              } else {
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      if (result?.data) {
        ...
        toast.error('Tidak dapat menemukan QR Code di gambar!');
      }
    } catch {
      toast.error('Gagal membaca QR Code dari gambar. Pastikan gambar jelas dan mengandung QR Code.');
    } finally {
      setLoading(false);
  ...
  const handleClearProduct = () => setScannedProduct(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { icon: Clock,   color: 'blue',   value: stats.todayScans,    label: 'Scan Hari Ini' },
          { icon: QrCode,  color: 'purple', value: stats.totalScans,    label: 'Total Scan' },
          { icon: Package, color: 'green',  value: stats.uniqueProducts, label: 'Produk Unik' },
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <button
          ...
          className={`flex flex-col items-center justify-center gap-3 p-6 rounded-xl border-2 transition-all ${
            cameraActive ? 'bg-red-50 border-red-300 hover:bg-red-100' : 'bg-blue-50 border-blue-300 hover:bg-blue-100'
          }`}
        >
          {cameraActive ? (
            <>
      ...
      {cameraActive && (
        <div className="bg-white rounded-xl shadow-lg border-2 border-blue-300 overflow-hidden">
          ...
          <div className="relative bg-black">
            <video ref={videoRef} className="w-full h-96 object-cover" playsInline />
          ...
          <div className="px-6 py-3 bg-gray-50 border-t border-gray-200">
            <p className="text-sm text-gray-600 text-center">📷 Arahkan kamera ke QR Code atau Barcode</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Kode QR / Barcode</label>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            {scannedProduct && (
              <button onClick={handleClearProduct} className="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-all" title="Clear">
                <X className="w-5 h-5" />
              </button>
            )}
          </div>
          ...
          {!scannedProduct ? (
            ...
            <div className="space-y-4">
              <div className="p-4 bg-gradient-to-r from-green-50 to-green-100 border-2 border-green-300 rounded-xl">
                ...
                <h4 className="text-xl font-bold text-green-900 mb-1">{scannedProduct.nama_barang}</h4>
                <p className="text-sm text-green-700">Kode: {scannedProduct.kode_barang}</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-xs text-gray-500 mb-1">Jenis Barang</p>
                  <p className="font-semibold text-gray-900">{scannedProduct.jenis_barang || '-'}</p>
                </div>
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-xs text-gray-500 mb-1">Satuan</p>
                  <p className="font-semibold text-gray-900">{scannedProduct.satuan || '-'}</p>
                </div>
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="text-xs text-blue-600 mb-1">Stok Tersedia</p>
                  <p className="text-2xl font-bold text-blue-700">{scannedProduct.stok || 0}</p>
                </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                        <span className="flex items-center gap-1"><Barcode className="w-3 h-3" />{history.kodeBarang}</span>
                        <span className="flex items-center gap-1"><User className="w-3 h-3" />{history.scannedBy}</span>
                      </div>
                    </div>
                  </div>
                  <p className="text-xs text-gray-500">{formatDate(history.scannedAt)}</p>
                </div>
              </div>
            ))
          )}
